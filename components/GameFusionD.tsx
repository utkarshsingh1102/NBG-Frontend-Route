...
// ── Option D Modal: compact inline dropdown selector ──────────────────────
function SelectModal({
  ...
  const searchFiltered = useMemo(() => {
    ...
    // When searching, also apply category filter if not "All"
    return allAvailable.filter((g) => {
      const matchesSearch = g.title.toLowerCase().includes(q);
      const matchesCat = modalCategory === "All" || g.category === modalCategory;
      return matchesSearch && matchesCat;
    });
  }, [search, allAvailable, modalCategory]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-[11px] text-amber-600 font-medium mt-0.5">Option D · Compact dropdown category selector</p>
          </div>
        ...
        {/* Search + Dropdown on same row (Option D key feature) */}
        <div className="px-6 pt-4 pb-3">
          <div className="flex items-center gap-2">
            ...
            {/* ── Compact dropdown (Option D key feature) ── */}
            <div className="relative flex-shrink-0">
              <select
                value={modalCategory}
                onChange={(e) => setModalCategory(e.target.value)}
                className="appearance-none pl-3 pr-7 py-2.5 text-[12px] font-semibold bg-white border-2 border-amber-300 rounded-xl focus:outline-none focus:border-amber-500 text-gray-700 cursor-pointer hover:border-amber-400 transition"
              ...
              <svg className="absolute right-2 top-1/2 -translate-y-1/2 text-amber-500 pointer-events-none" width="12" height="12" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
            </div>
          </div>

          <p className="text-xs mt-2 text-gray-400">
            {search.trim()
              ? `${searchFiltered.length} result${searchFiltered.length !== 1 ? "s" : ""} found${modalCategory !== "All" ? ` in ${modalCategory}` : ""}`
              : modalCategory === "All"
              ? `${allGames.length} system games · ${userGames.length} your game${userGames.length !== 1 ? "s" : ""}`
              : `${systemFiltered.length} ${modalCategory} games · ${allGames.length - systemFiltered.length} hidden`}
          ...
          {modalCategory !== "All" && (
            <div className="flex items-center gap-1.5 mt-2">
              <span className="text-[10px] text-gray-400">Filtering:</span>
              <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 border border-amber-200">
                {modalCategory}
                <button onClick={() => setModalCategory("All")} className="ml-0.5 hover:text-red-500 transition">
          ...
          {search.trim() ? (
            searchFiltered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-gray-400">
                ...
                <p className="text-sm">No {modalCategory !== "All" ? `${modalCategory} ` : ""}games found for &quot;{search}&quot;</p>
            ...
            /* Sectioned view — System Games filtered by dropdown */
            <>
              <div>
              ...
              <div>
                <button onClick={() => setSystemSectionOpen((o) => !o)} className="flex items-center justify-between w-full px-6 py-3 bg-gray-50 hover:bg-gray-100 transition text-left border-b border-gray-100">
                  <div className="flex items-center gap-2">
                    ...
                    {modalCategory !== "All" && <span className="text-[9px] text-amber-500 font-medium">· {modalCategory} filter active</span>}
                  </div>
                ...
                {systemSectionOpen && <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 px-6 pt-4">{systemFiltered.map((g) => <Card key={g.id} game={g} />)}</div>}
              </div>
            </>
          )}
        </div>
...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function GameFusionD({ activeCategory, onCategoryChange, userGames, onNavigateToAddGame }: Props) {

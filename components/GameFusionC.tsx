...
// ── Option C Modal: smart filter — pills + flat grid when category active ──
function SelectModal({
  ...
  // Flat grid shown when a specific category is selected (not "All") and no search
  const categoryGames = useMemo(() => {
    if (modalCategory === "All") return [];
  ...
  function Card({ game }: { game: Game }) {
    ...
    return (
      <button onClick={() => { if (disabled) return; if (multiSelect) togglePending(game.id); else onSelect?.(game); }}
        className={`relative rounded-xl overflow-hidden transition-all duration-150 ${selected ? "ring-[3px] ring-amber-400 ring-offset-1 scale-[0.98]" : disabled ? "opacity-40 cursor-not-allowed" : "hover:scale-[1.03] hover:shadow-md"}`}>
        ...
        {userGames.includes(game) && <span className="absolute top-1.5 left-1.5 bg-amber-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-full leading-none">YOURS</span>}
  ...
  const isCategoryMode = !search.trim() && modalCategory !== "All";
  const isSectionMode = !search.trim() && modalCategory === "All";
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-[11px] text-amber-600 font-medium mt-0.5">Option C · Smart filter: sections in All mode, flat grid in category mode</p>
          </div>
        ...
        <div className="px-6 pt-4 pb-2">
          <div className="relative">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 w-4 h-4 pointer-events-none" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <input type="text" placeholder="Search games..." autoFocus value={search} onChange={(e) => { setSearch(e.target.value); if (e.target.value) setModalCategory("All"); }}
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <button key={cat} onClick={() => setModalCategory(cat)}
                    className={`flex items-center gap-1 text-[11px] px-2.5 py-1 rounded-full border font-semibold transition-all ${isActive ? "bg-amber-500 text-white border-amber-500 shadow-sm" : "bg-white text-gray-500 border-gray-200 hover:border-amber-300 hover:text-amber-600"}`}>
                    {cat}
                    <span className={`text-[9px] font-bold px-1 py-0.5 rounded-full leading-none ${isActive ? "bg-white/25 text-white" : "bg-gray-100 text-gray-500"}`}>{count}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="flex items-center justify-between mt-2">
            <p className="text-xs text-gray-400">
              {search.trim()
                ...
                : isCategoryMode
                ? <span><span className="text-amber-600 font-semibold">{categoryGames.length}</span> {modalCategory} games · flat grid view</span>
            ...
            {isCategoryMode && (
              <button onClick={() => setModalCategory("All")} className="text-[10px] font-semibold text-amber-600 hover:text-amber-700 flex items-center gap-1 transition">
          ...
          {search.trim() ? (
            ...
            searchFiltered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 gap-3 text-gray-400">
                ...
                {onNavigateToAddGame && <button onClick={() => { onClose(); onNavigateToAddGame(); }} className="flex items-center gap-2 px-4 py-2 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-xl text-sm font-semibold text-amber-700 transition"><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>Add &quot;{search}&quot; as a new game</button>}
              </div>
            ) : (
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 px-6">{searchFiltered.map((g) => <Card key={g.id} game={g} />)}</div>
            )
          ) : isCategoryMode ? (
            /* ── Category mode: flat focused grid ── */
            <div>
              <div className="px-6 py-3 bg-amber-50 border-b border-amber-100 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-amber-400" />
                <span className="text-[11px] font-bold text-amber-800 uppercase tracking-wider">{modalCategory}</span>
                <span className="text-[10px] text-amber-600">{categoryGames.length} games</span>
              </div>
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 px-6 pt-4">
                {categoryGames.map((g) => <Card key={g.id} game={g} />)}
              </div>
            </div>
          ) : (
            /* ── All mode: normal sections view ── */
            <>
              <div>
                <button onClick={() => setUserSectionOpen((o) => !o)} className="flex items-center justify-between w-full px-6 py-3 bg-gray-50 hover:bg-gray-100 transition text-left border-b border-gray-100">
                  <div className="flex items-center gap-2"><span className="text-[11px] font-bold text-gray-700 uppercase tracking-wider">Your Games</span><span className="text-[10px] bg-amber-100 text-amber-700 font-semibold px-1.5 py-0.5 rounded-full">{userGames.length}</span></div>
                ...
                {userSectionOpen && (
                  <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 px-6 pt-4 pb-2">
                    {userGames.map((g) => <Card key={g.id} game={g} />)}
                    {onNavigateToAddGame && <button onClick={() => { onClose(); onNavigateToAddGame(); }} className="relative rounded-xl overflow-hidden border-2 border-dashed border-amber-200 hover:border-amber-400 bg-amber-50 hover:bg-amber-100 flex flex-col items-center justify-center gap-1.5 aspect-square transition-all group"><div className="w-8 h-8 rounded-lg bg-amber-200 group-hover:bg-amber-300 flex items-center justify-center"><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#d97706" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg></div><p className="text-[9px] font-bold text-amber-700 text-center leading-tight px-1">Add a Game</p></button>}
                  </div>
                )}
              </div>
              <div>
                <button onClick={() => setSystemSectionOpen((o) => !o)} className="flex items-center justify-between w-full px-6 py-3 bg-gray-50 hover:bg-gray-100 transition text-left border-b border-gray-100">
                  <div className="flex items-center gap-2"><span className="text-[11px] font-bold text-gray-700 uppercase tracking-wider">System Games</span><span className="text-[10px] bg-gray-200 text-gray-600 font-semibold px-1.5 py-0.5 rounded-full">{allGames.length}</span></div>
                ...
                {systemSectionOpen && <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 px-6 pt-4">{allGames.map((g) => <Card key={g.id} game={g} />)}</div>}
              </div>
            </>
          )}
        </div>
...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function GameFusionC({ activeCategory, onCategoryChange, userGames, onNavigateToAddGame }: Props) {
  ...
  const handleGenerate = () => { setToast(true); setTimeout(() => { setToast(false); setSourceGame(null); setTargetGames([]); }, 2000); };
    ...
    <>
      <main className="flex-1 p-6 min-w-0 space-y-14">
        <section className="bg-white rounded-2xl p-5 border border-gray-100 shadow-md">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-xl bg-amber-500 flex items-center justify-center shadow-sm shadow-amber-200"><svg width="15" height="15" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth={2.2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg></div>
              <div><h2 className="text-base font-bold text-gray-900">Popular Game Combos</h2><p className="text-[11px] text-amber-700/70">Click any combo to auto-select both games</p></div>
            </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap mb-3">
            {COMBO_CATEGORIES.map((cat) => (
              <button key={cat} onClick={() => setComboCategory(cat)} className={`text-[11px] px-3 py-1 rounded-full border font-semibold transition-all ${comboCategory === cat ? "bg-amber-500 text-white border-amber-500 shadow-sm" : "bg-white text-gray-500 border-gray-200 hover:border-amber-300 hover:text-amber-600"}`}>{cat}</button>
            ))}
          </div>
          <div className="flex items-stretch gap-0 overflow-x-auto pb-1 scrollbar-hide" style={{ flexWrap: "nowrap" }}>
            {POPULAR_COMBOS.filter((c) => comboCategory === "All" || c.group === comboCategory).map((combo, idx, arr) => {
              ...
              const isNewGroup = idx > 0 && arr[idx - 1].group !== combo.group;
              return (
                <div key={`${combo.game1Id}-${combo.game2Id}`} className="flex items-stretch gap-0 flex-shrink-0">
                  ...
                  <button onClick={() => { setSourceGame(g1); setTargetGames([g2]); }} className="bg-white border border-gray-100 rounded-xl p-3 mx-1 hover:border-amber-300 hover:shadow-lg hover:shadow-amber-100 transition-all group w-fit flex flex-col items-center">
        ...
        <section className="bg-gradient-to-br from-amber-50 via-amber-100 to-orange-100 rounded-2xl p-6 shadow-lg shadow-amber-100/60 border border-amber-200/60">
          <div className="mb-4 text-center"><h1 className="text-xl font-bold text-gray-900">Select Game or App</h1><p className="text-gray-500 text-xs mt-0.5">Pick a source game, then add one or more target games to compare</p></div>
          <div className="flex gap-3 h-[120px] justify-center items-center">
            <button onClick={() => setSourceModalOpen(true)} className="w-[120px] flex-shrink-0 h-full rounded-2xl border-2 border-dashed border-amber-200 bg-white hover:border-amber-400 transition-all group overflow-hidden shadow-sm">
              {sourceGame ? <div className="flex flex-col items-center justify-center h-full gap-1.5 px-2"><img src={thumbSquare(sourceGame.title, sourceGame.category, allGames.indexOf(sourceGame))} alt={sourceGame.title} className="w-14 h-14 rounded-xl object-cover" /><div className="text-center w-full"><p className="text-[9px] font-semibold text-amber-600 mb-0.5">Source</p><p className="text-[11px] font-bold text-gray-900 truncate">{sourceGame.title}</p><span className="text-[9px] text-gray-400 group-hover:text-amber-600 transition">Tap to change →</span></div></div>
              : <div className="flex flex-col items-center justify-center h-full gap-2"><div className="w-10 h-10 rounded-xl bg-amber-100 group-hover:bg-amber-200 flex items-center justify-center transition-colors"><svg width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="#f59e0b" strokeWidth={1.8}><path strokeLinecap="round" strokeLinejoin="round" d="M6 12h4M8 10v4M14 11h.01M17 13h.01M5 8h14a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2z" /></svg></div><div className="text-center"><p className="text-xs font-semibold text-gray-800">Source Game</p><p className="text-[10px] text-gray-500">Choose base game</p></div></div>}
            </button>
            ...
            <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
              {targetGames.map((game) => (
                <div key={game.id} className="relative flex-shrink-0 w-[120px] h-[120px] rounded-2xl overflow-hidden border border-amber-200 shadow-sm group/card">
                  ...
                  <button onClick={() => removeTarget(game.id)} className="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/50 flex items-center justify-center opacity-0 group-hover/card:opacity-100 transition-opacity hover:bg-red-500"><svg width="8" height="8" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
                </div>
              ))}
          ...
          <div className="mt-5 flex justify-center"><button onClick={handleGenerate} className="flex items-center gap-2 px-6 py-2.5 bg-gray-900 hover:bg-gray-800 text-white text-sm font-semibold rounded-xl transition shadow-md"><svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>Generate Now</button></div>
        </section>

        {toast && <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-2.5 bg-gray-900 text-white text-sm font-medium px-5 py-3 rounded-2xl shadow-xl animate-fade-in"><svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#f59e0b" strokeWidth={2.5} className="animate-spin"><path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>Generating the Idea...</div>}

        <RandomRemixGenerator />

        <section>
          <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div><h2 className="text-base font-bold text-gray-900">My Discovered Games</h2><p className="text-xs text-gray-400 mt-0.5"><span className="text-amber-600 font-semibold">{publishedCount}</span> published · <span className="text-gray-500 font-medium">{allGames.length - publishedCount}</span> drafts · <span className="font-medium text-gray-600">{filtered.length} showing</span></p></div>
            <div className="flex gap-2"><button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition"><svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>Import</button><button className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-amber-500 rounded-lg hover:bg-amber-600 transition shadow-sm"><svg width="13" height="13" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>New Game</button></div>
          </div>

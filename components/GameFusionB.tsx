...
const CATEGORIES = ["Puzzle", "Casual", "Arcade", "Runner", "Match3", "Strategy", "Merge", "Idle", "Hypercasual", "Simulation"];
const COMBO_CATEGORIES = ["All", ...CATEGORIES];

// ── Option B Modal: category-grouped sections replace Your/System ──────────
function SelectModal({
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>(() =>
    Object.fromEntries([...CATEGORIES, "Your Games"].map((c) => [c, true]))
  );
  const MAX_TARGETS = 5;
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-[11px] text-amber-600 font-medium mt-0.5">Option B · Games grouped by category</p>
          </div>
        ...
        <div className="px-6 pt-4 pb-3">
          ...
          <p className="text-xs mt-2 text-gray-400">
            {search.trim() ? `${searchFiltered.length} result${searchFiltered.length !== 1 ? "s" : ""} found` : `${allGames.length} games in ${CATEGORIES.length} categories`}
        ...
        <div className="flex-1 overflow-y-auto pb-6">
          {search.trim() ? (
            ...
            <>
              {/* ── Your Games section ── */}
              {(userGames.length > 0 || onNavigateToAddGame) && (
                <div>
                  <button onClick={() => toggleSection("Your Games")} className="flex items-center justify-between w-full px-6 py-3 bg-amber-50 hover:bg-amber-100/70 transition text-left border-b border-amber-100">
                    <div className="flex items-center gap-2">
                      <span className="w-2 h-2 rounded-full bg-amber-400 flex-shrink-0" />
                      <span className="text-[11px] font-bold text-amber-800 uppercase tracking-wider">Your Games</span>
                      <span className="text-[10px] bg-amber-200 text-amber-800 font-semibold px-1.5 py-0.5 rounded-full">{userGames.length}</span>
                    </div>
                    <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} className={`text-amber-400 transition-transform ${openSections["Your Games"] ? "rotate-180" : ""}`}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                  </button>
                  {openSections["Your Games"] && (
              ...
              {CATEGORIES.map((cat, catIdx) => {
                ...
                const colors = ["bg-violet-50 border-violet-100 text-violet-800", "bg-green-50 border-green-100 text-green-800", "bg-blue-50 border-blue-100 text-blue-800", "bg-rose-50 border-rose-100 text-rose-800", "bg-fuchsia-50 border-fuchsia-100 text-fuchsia-800", "bg-cyan-50 border-cyan-100 text-cyan-800", "bg-teal-50 border-teal-100 text-teal-800", "bg-orange-50 border-orange-100 text-orange-800", "bg-indigo-50 border-indigo-100 text-indigo-800", "bg-emerald-50 border-emerald-100 text-emerald-800"];
                const dotColors = ["bg-violet-400", "bg-green-400", "bg-blue-400", "bg-rose-400", "bg-fuchsia-400", "bg-cyan-400", "bg-teal-400", "bg-orange-400", "bg-indigo-400", "bg-emerald-400"];
                const colorClass = colors[catIdx % colors.length];
                const dotClass = dotColors[catIdx % dotColors.length];
                return (
                  <div key={cat}>
                    <button onClick={() => toggleSection(cat)} className={`flex items-center justify-between w-full px-6 py-3 hover:opacity-80 transition text-left border-b ${colorClass}`}>
                      <div className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full flex-shrink-0 ${dotClass}`} />
                        <span className="text-[11px] font-bold uppercase tracking-wider">{cat}</span>
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-white/60">{catGames.length}</span>
                      </div>
                      <svg width="14" height="14" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} className={`transition-transform opacity-60 ${openSections[cat] ? "rotate-180" : ""}`}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    {openSections[cat] && (
                      <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 px-6 pt-4 pb-2">
                        {catGames.map((g) => <Card key={g.id} game={g} />)}
                      </div>
                    )}
                  </div>
                );
...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function GameFusionB({ activeCategory, onCategoryChange, userGames, onNavigateToAddGame }: Props) {

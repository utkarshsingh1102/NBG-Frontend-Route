...
const CATEGORY_COLORS: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  Puzzle:      { bg: "bg-violet-50",  text: "text-violet-700",  border: "border-violet-300",  dot: "bg-violet-400"  },
  Casual:      { bg: "bg-green-50",   text: "text-green-700",   border: "border-green-300",   dot: "bg-green-400"   },
  Arcade:      { bg: "bg-blue-50",    text: "text-blue-700",    border: "border-blue-300",    dot: "bg-blue-400"    },
  Runner:      { bg: "bg-rose-50",    text: "text-rose-700",    border: "border-rose-300",    dot: "bg-rose-400"    },
  Match3:      { bg: "bg-fuchsia-50", text: "text-fuchsia-700", border: "border-fuchsia-300", dot: "bg-fuchsia-400" },
  Strategy:    { bg: "bg-cyan-50",    text: "text-cyan-700",    border: "border-cyan-300",    dot: "bg-cyan-400"    },
  Merge:       { bg: "bg-teal-50",    text: "text-teal-700",    border: "border-teal-300",    dot: "bg-teal-400"    },
  Idle:        { bg: "bg-orange-50",  text: "text-orange-700",  border: "border-orange-300",  dot: "bg-orange-400"  },
  Hypercasual: { bg: "bg-indigo-50",  text: "text-indigo-700",  border: "border-indigo-300",  dot: "bg-indigo-400"  },
  Simulation:  { bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-300", dot: "bg-emerald-400" },
};

// ── Modal: category pre-selected, same tab filter style as Fusion A ──────────
function SelectModal({
  ...
  const systemFiltered = useMemo(
    () => (modalCategory === "All" ? allGames : allGames.filter((g) => g.category === modalCategory)),
    [modalCategory]
  );

  const userFiltered = useMemo(
    () => (modalCategory === "All" ? userGames : userGames.filter((g) => g.category === modalCategory)),
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-[11px] text-amber-600 font-medium mt-0.5">
              Fusion E · Category-First — pre-filtered to{" "}
              <span className="font-bold">{initialCategory}</span>
            </p>
          </div>
          <div className="flex items-center gap-2">
            {multiSelect && (
              <button
                onClick={() => onMultiSelect?.(allAvailable.filter((g) => pending.includes(g.id)))}
            ...
            <div className="flex items-center gap-1.5 flex-wrap mt-3">
              {["All", ...CATEGORIES].map((cat) => {
                ...
                return (
                  ...
                  >
                    {cat}
                    <span className={`text-[9px] font-bold px-1 py-0.5 rounded-full leading-none ${modalCategory === cat ? "bg-white/25 text-white" : "bg-gray-100 text-gray-500"}`}>
                      {count}
                    </span>
                  </button>
                );
          ...
          {search.trim() ? (
            searchFiltered.length === 0 ? (
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 px-6">
                {searchFiltered.map((g) => <Card key={g.id} game={g} />)}
              </div>
            )
          ) : (
            <>
                  ...
                  <div className="grid grid-cols-4 sm:grid-cols-5 gap-3 px-6 pt-4 pb-2">
                    ...
                    {onNavigateToAddGame && (
                      <button onClick={() => { onClose(); onNavigateToAddGame(); }} className="relative rounded-xl overflow-hidden border-2 border-dashed border-amber-200 hover:border-amber-400 bg-amber-50 hover:bg-amber-100 flex flex-col items-center justify-center gap-1.5 aspect-square transition-all group">
                        <div className="w-8 h-8 rounded-lg bg-amber-200 group-hover:bg-amber-300 flex items-center justify-center">
                          <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#d97706" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
                        </div>
...
// ── Page ──────────────────────────────────────────────────────────────────────
export default function GameFusionE({ activeCategory, onCategoryChange, userGames, onNavigateToAddGame }: Props) {
  ...
  // ── Category gate: must pick before game selection is enabled ──
  const [pickedCategory, setPickedCategory] = useState<string | null>(null);

  const filtered = useMemo(
    () => (activeCategory === "All" ? allGames : allGames.filter((g) => g.category === activeCategory)),
    [activeCategory]
  );
  ...
  const handleCategoryPick = (cat: string) => {
    // Deselect if same category clicked again
    if (pickedCategory === cat) {
      setPickedCategory(null);
      setSourceGame(null);
      setTargetGames([]);
    } else {
      setPickedCategory(cat);
      // Clear previously selected games when category changes
      setSourceGame(null);
      setTargetGames([]);
    }
  };
  ...
  const colors = CATEGORY_COLORS[pickedCategory ?? "Puzzle"] ?? CATEGORY_COLORS["Puzzle"];
  const isGated = !pickedCategory;
  ...
  return (
    <>
      <main className="flex-1 p-6 min-w-0 space-y-14">
        ...
        <section className="bg-white rounded-2xl p-5 border border-gray-100 shadow-md">
          <div className="flex items-center justify-between mb-4">
            ...
            <button className="text-xs text-amber-600 font-semibold hover:text-amber-700 transition bg-white/70 px-3 py-1.5 rounded-lg border border-amber-200 hover:bg-white">
              View all →
            </button>
          </div>
            ...
            {POPULAR_COMBOS.filter((c) => comboCategory === "All" || c.group === comboCategory).map((combo, idx, arr) => {
              ...
              return (
                <div key={`${combo.game1Id}-${combo.game2Id}`} className="flex items-stretch gap-0 flex-shrink-0">
                  ...
                  <button
                    onClick={() => {
                      // Auto-pick category from the combo's group
                      setPickedCategory(combo.group);
                      setSourceGame(g1);
                      setTargetGames([g2]);
                    }}
        ...
        {/* ── Select Game — with category gate ── */}
        <section className="bg-gradient-to-br from-amber-50 via-amber-100 to-orange-100 rounded-2xl p-6 shadow-lg shadow-amber-100/60 border border-amber-200/60">
          <div className="mb-5 text-center">
            ...
            <p className="text-gray-500 text-xs mt-0.5">
              First choose a category below, then pick your source and target games
            </p>
          </div>
          ...
          <div className="mb-5">
            <div className="flex items-center gap-2 mb-3">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${pickedCategory ? "bg-green-500 text-white" : "bg-amber-500 text-white"}`}>
                {pickedCategory ? (
              ...
              <span className="text-[12px] font-bold text-gray-700 uppercase tracking-wide">
                {pickedCategory ? `Category: ${pickedCategory}` : "Choose a category to begin"}
              </span>
              {pickedCategory && (
                <button
                  onClick={() => handleCategoryPick(pickedCategory)}
                  className="ml-auto text-[10px] font-semibold text-gray-400 hover:text-red-500 transition flex items-center gap-1"
                >
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {CATEGORIES.map((cat) => {
                ...
                const isActive = pickedCategory === cat;
                return (
                  <button
                    key={cat}
                    onClick={() => handleCategoryPick(cat)}
                    className={`flex items-center gap-1.5 text-[12px] px-3 py-1.5 rounded-xl border-2 font-semibold transition-all ${
                      isActive
                        ? `${c.bg} ${c.text} ${c.border} shadow-sm scale-105`
                        : "bg-white text-gray-500 border-gray-200 hover:border-gray-300 hover:text-gray-700"
                    }`}
                  >
                    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${isActive ? c.dot : "bg-gray-300"}`} />
                    {cat}
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full leading-none ${isActive ? "bg-white/50" : "bg-gray-100 text-gray-400"}`}>
                      {count}
                    </span>
                  </button>
                );
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <div className="flex items-center gap-1.5">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${isGated ? "bg-gray-300 text-gray-500" : sourceGame ? "bg-green-500 text-white" : "bg-amber-500 text-white"}`}>
                {!isGated && sourceGame ? (
                  <svg width="10" height="10" fill="none" viewBox="0 0 24 24" stroke="white" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                ) : "2"}
              </div>
              <span className={`text-[12px] font-bold uppercase tracking-wide ${isGated ? "text-gray-400" : "text-gray-700"}`}>
                {isGated ? "Select games (locked)" : "Select games"}
              </span>
              {isGated && (
          ...
          {/* ── Step 2: Game selectors (gated) ── */}
          <div className={`transition-all duration-300 ${isGated ? "opacity-40 pointer-events-none select-none" : "opacity-100"}`}>
            {/* Active category banner */}
            {pickedCategory && (
              <div className={`flex items-center gap-2 mb-3 px-3 py-2 rounded-xl ${colors.bg} border ${colors.border}`}>
                <span className={`w-2 h-2 rounded-full ${colors.dot}`} />
                <span className={`text-[11px] font-bold ${colors.text}`}>
                  Showing {allGames.filter((g) => g.category === pickedCategory).length} {pickedCategory} games
                </span>
                <span className={`ml-auto text-[9px] font-semibold ${colors.text} opacity-70`}>
                  Select from this category only
                </span>
              </div>
            )}
            ...
            <div className="flex gap-3 h-[120px] justify-center items-center">
              {/* Source */}
              <button
                onClick={() => !isGated && setSourceModalOpen(true)}
              ...
              <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide">
                ...
                <button
                  onClick={() => !isGated && setTargetModalOpen(true)}
                ...
                >
                  <div className="w-8 h-8 rounded-lg bg-amber-100 group-hover:bg-amber-200 flex items-center justify-center">
                    <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#f59e0b" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
                  </div>
                  <p className="text-[10px] font-semibold text-gray-700 group-hover:text-amber-700 text-center leading-tight">
          ...
          {/* Locked hint */}
          {isGated && (
            <p className="text-center text-[11px] text-amber-700/80 font-medium mt-3">
              ↑ Pick a category above to unlock game selection
            </p>
          )}
      ...
      {sourceModalOpen && (
        <SelectModal
          slot={1}
          initialCategory={pickedCategory ?? "All"}
          selectedId={sourceGame?.id}
          userGames={userGames}
          onNavigateToAddGame={() => { setSourceModalOpen(false); onNavigateToAddGame(); }}
          onSelect={(g) => { setSourceGame(g); setSourceModalOpen(false); }}
      ...
      {targetModalOpen && (
        <SelectModal
          ...
          onMultiSelect={(gs) => { setTargetGames(gs); setTargetModalOpen(false); }}
